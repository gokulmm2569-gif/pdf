...
def test_api():
    ...
    with open('sample_invoices/imran_air_express_text.pdf', 'rb') as f:
        pdf_data = f.read()
    ...
    with urllib.request.urlopen(req) as resp:
        ...
        print("HEADER EXTRACTED:", data.get("header"))
        
        # Test download
        dl_url = f"http://127.0.0.1:8000{data.get('download_url')}"
        with urllib.request.urlopen(dl_url) as dl_resp:
            excel_bytes = dl_resp.read()
            print("EXCEL DOWNLOAD STATUS:", dl_resp.status)
            print("EXCEL BYTES RECEIVED:", len(excel_bytes))
